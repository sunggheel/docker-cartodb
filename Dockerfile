...
#ENV CARTODB_VERSION=4.40.0
ENV CARTODB_VERSION=master
#ENV SQLAPI_VERSION=7.0.0
...
#    apt-get remove -y ruby2.5-dev && \
    apt-get update && apt-get install -y ruby ruby-bundler && \
    gem install bundler -v "$(grep -A 1 "BUNDLED WITH" Gemfile.lock | tail -n 1)" && gem install compass && \
